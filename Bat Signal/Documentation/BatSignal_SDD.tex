...
The BatSignal distributed sensor network relies heavily on the ability for nodes to be physically separate
...
\textnormal{Input to the system is through configuration files, the administration console, and audio.  Configuration files are read by each node on startup.  The administration console can be used at anytime to configure the system.  There are two files that are used for configuration: the help phrase file, and the administrators file.  These files are stored on the control node in a subdirectory of the installation folder.  The installation folder is the location where the python modules for the system have been installed.  The files are stored in the subdirectory named "config". The files are stored as "phrases.json" and "admins.json".  They contain a single json object containing a list of strings.  The list of string for "phrases.json" is the help phrases for the system.  The phrases can be any single english word. The list of strings for "admins.json" are email addresses for the administrators.  All administrators listed in the configuration file recieve an email when the system detects a help phrase. \\
The input to the sensor nodes is audio signals recorded using the attached microphone.  The control node receives input in the form of text messages relayed across the mesh network from the sensor nodes.}
% Need an elaboration here concerning the administration console.
...
% •	Description of any access restrictions or security considerations
\textnormal{Sensor nodes produce text messages as output.  The text messages contain the converted audio into english text.  The control node produces the final output in the form of email messages to the administrators when a help phrase is detected.}
