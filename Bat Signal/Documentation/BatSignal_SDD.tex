...
Python modules installed on the sensor nodes passively read input from a microphone. The input from the microphone is fed into python's speech recognition module. The audio input is then sent to the control node over the mesh network either as plain text or compressed plain text. \\\\
